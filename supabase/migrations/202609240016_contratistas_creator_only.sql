...
-- The shared contractor directory is managed only from the Creator workspace.
...
begin
 ...
 or not exists(select 1 from public.tenants t where t.id=p_tenant and t.activo) then
   raise exception 'Solo el Creador puede administrar el directorio de contratistas' using errcode='42501';
 end if;
 if p_nombre is null or length(btrim(p_nombre)) not between 1 and 160 then
   raise exception 'El nombre debe tener entre 1 y 160 caracteres' using errcode='22023';
 end if;
...
begin
 ...
 if tg_table_name='ordenes' and tg_op<>'DELETE' and tid is not null and exists(
   select 1 from jsonb_array_elements_text(coalesce(nullif(r->'contratistas','null'::jsonb),'[]'::jsonb)) v
   where length(btrim(v)) not between 1 and 160 or not exists(
     select 1 from public.plan_contratistas c where c.tenant_id=tid and c.nombre_clave=lower(btrim(v)))) then
   raise exception 'Los contratistas deben elegirse del directorio administrado por el Creador' using errcode='22023';
 end if;
 if d<>'{}'::jsonb then
