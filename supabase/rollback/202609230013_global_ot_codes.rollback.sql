...
drop trigger if exists plan_ot_codigo_guardar on public.ordenes;
drop function if exists public.plan_ot_codigo_guardar();
drop index if exists public.ordenes_ot_global_uidx;
drop sequence if exists public.plan_ot_codigo_seq;
-- Los códigos ya asignados se conservan. La versión anterior de restauración
-- solo se recupera después de quitar la unicidad global.
