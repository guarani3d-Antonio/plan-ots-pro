...
| **6** | Adaptar frontend a tenant y membresías reales: sesión, selector, permisos y cambio de cuenta. Aislar caché/cola y respuestas tardías; bloquear fallback inseguro. Proteger costos en servidor, incluidas vistas/snapshots. La creación por RPC ya quedó adaptada en día 4. | Cada rol ve solo sus obras; cambio de cuenta no revela ni sincroniza datos de otra identidad; Creador administra; técnicos/lectores no recuperan costos por API. Preparar enlace público actualizado tras estas comprobaciones. |
